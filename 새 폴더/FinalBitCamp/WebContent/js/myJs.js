...
//	function settingMileage(mileage) {			//마일리지 radio 버튼 출력
...
//			}));
//		}
//		$('.radio-mileage').append($('<span>',{'class':'availible-mileage', text : '사용 가능한 마일리지 :'})).append(new_array[0]).append(new_array[1]);
//		$('.use-mileage').text('0원');
...
//	}
	...
	$('.using-mileage').on('change', function(){
		var mileage_type = $(this).val();
		if(mileage_type=='1'){
			if(get_mileage < 1000){
				alert('마일리지는 1000원 이상 부터 사용 가능합니다.');
			}else{
				mileage_total += parseInt(get_mileage);
				$('.use-mileage').text(get_mileage.replace(/(\d)(?=(?:\d{3})+(?!\d))/g, '$1,') + '원');
				$('.availible-mileage').text('0원');
			}
		}else{
			mileage_total = 0;
			$('.use-mileage').text('0원');
